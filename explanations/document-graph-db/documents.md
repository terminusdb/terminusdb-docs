...
TerminusDB is a document store as well as a knowledge graph database. TerminusDB [schemata](../../guides/reference-guides/schema.md) describe how to interpret segments of graphs as self-contained documents.
...
See the [Subdocuments](documents.md#subdocuments) section for more information.

### Simple documents

Documents are described by creating a class definition in the [schema](../../guides/reference-guides/schema.md). The simplest document definition contains properties with data elements only.

#### A simple document example

The [Football roster CSV file](documents.md#data-football-roster-csv-file) is loaded using the [Football roster class definition](documents.md#code-football-roster-class-definition) below. This class definition describes the JSON document that can be submitted or retrieved from the graph.

A valid [Football roster JSON document definition](documents.md#code-football-roster-json-document-definition), which corresponds to a row in the CSV file, is also shown below.
...
It is important to ensure a unique document `@id`. The parameters `@key` and `@base` are available in the class definition, enabling you to use a calculated `@id`. Refer to the [Schema reference](../../guides/reference-guides/schema.md) for more information.
...
The [simple document example](documents.md#a-simple-document-example) introduced above is convertible to an object that refers to each player, as demonstrated in the code-snippet [Player and roster classes](documents.md#code-player-and-roster-classes). In this snippet, the `Roster` class points to a `Set` of `Player`s (or `Player` classes.)
...
A set of documents that meets the above specification and that represents the data in the [Football roster CSV file](documents.md#data-football-roster-csv-file) can be defined as:
...
An example schema representing this scenario is provided in the code-snippet [Contact graph schema](documents.md#code-contact-graph-schema) further below with an example of a corresponding [Contact graph document](documents.md#code-contact-graph-document) definition.
...
#### [TerminusDB Document Interface Reference Guide](../../guides/reference-guides/document-interface.md)

#### [TerminusDB Schema Reference Guide](../../guides/reference-guides/schema.md)
