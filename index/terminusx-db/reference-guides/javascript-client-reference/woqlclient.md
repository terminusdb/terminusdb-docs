...
| \[lastDataVersion] | `string`    | If passed it will be used for data version tracking                                                                                     |
| \[getDataVersion]  | `string`    | If true it the function will return object having result and dataVersion                                                                |
...
Use [#localAuth](broken-reference) instead.
...
Use [#remoteAuth](broken-reference) instead.
...
**woqlClient.copy() ⇒** [**`WOQLClient`**](broken-reference)
...
**Returns**: [`WOQLClient`](broken-reference) - new client object with identical state to original but which can be manipulated independently\
**Example**
...
**woqlClient.databaseInfo(\[dbId]) ⇒ `object`**
